#!/usr/bin/env python3
"""
Cloudflare Auto DNS Manager v6.4 (Token Protected - Final Pro Edition)
Author: MHR Dev Team 🌿
"""
...
def install_dependencies():
    ...
    packages = ["python3", "python3-pip", "curl", "git", "nano", "zip"]
    for pkg in packages:
        run_cmd(f"apt install -y {pkg} >/dev/null 2>&1")
...
def safe_request(action, *args, retries=4, delay=1.5, **kwargs):
    ...
    for i in range(retries):
        try:
            ...
            print(f"{YELLOW}⚠️ Retry {i+1}/{retries}: {e}{RESET}")
            time.sleep(delay)
            delay *= 1.5
    raise
...
def action_delete(sess, zone, domain):
    ...
    cnt = int(input("How many to delete?: ") or "0")
    if cnt <= 0:
    ...
    for i in range(1, cnt+1):
        ...
        try:
            ...
            if j.get("result"):
                ...
                print(f"{RED}🗑️ Deleted {name}{RESET}")
                deleted += 1
            else:
                print(f"{YELLOW}⚠️ Not found {name}{RESET}")
        except Exception as e:
            print(f"{RED}❌ {e}{RESET}")
...
def action_list_pro_compact(sess, zone, domain):
    ...
    recs = list_all_records(sess, zone)
    if not recs:
        print(f"{YELLOW}⚠️ No DNS records found for this zone.{RESET}")
        return
...
""")
        ...
        if choice == "1":
            action_create(sess, zone, domain)
        elif choice == "2":
            action_delete(sess, zone, domain)
        elif choice == "3":
            action_list(sess, zone, domain)
        elif choice == "4":
            action_list_pro_compact(sess, zone, domain)
        elif choice == "5":
